...
def update(agent,data,B,geometry="fisher",metric="KL",scale=1.,c_w=1.,estimator="time_loo",actor_reward_target="shaped"):
 assert metric in ["KL","W2"] and c_w>0
 assert estimator=="time_loo" and actor_reward_target in ["shaped","raw"]
 radius=.01*scale
 start=time.time();H=12;N=len(data['states'])//H
 done=data['dones'].reshape(N,H);assert done[:,-1].bool().all() and not done[:,:-1].bool().any()
 rt=finite_targets(data['rewards']);ct=finite_targets(data['costs']);actor_rt=rt if actor_reward_target=='shaped' else finite_targets(data['raw_rewards']);ar=centered(actor_rt).double();ac=centered(ct).double();agent.audit_reward_baseline=dict(estimator=estimator,actor_reward_target=actor_reward_target,targets=actor_rt.detach().cpu(),critic_targets=rt.detach().cpu(),advantage=ar.detach().cpu())
 ...
 info.update(actor_reward_target=actor_reward_target,actor_target_return_mean=float(actor_rt.reshape(N,H)[:,0].mean()),reward_estimator=estimator,baseline_seconds=agent.audit_reward_baseline.get("seconds",0.),reward_advantage_std=float(ar.std()),reward_advantage_mean=float(ar.mean()),cost_advantage_std=ac.std(0).tolist())
 return info

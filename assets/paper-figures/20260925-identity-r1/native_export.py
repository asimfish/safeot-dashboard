...
def freeform_loop(sl,points):
 ...
 sh.fill.background();sh.line.color.rgb=RGBColor.from_string('64717C');sh.line.width=Pt(.7)
 ln=sp.find('{'+math_tools.ANS+'}ln');end=OxmlElement('a:tailEnd');end.set('type','triangle');end.set('w','sm');end.set('len','sm');ln.append(end)
 math_tools.clean(sh);return sh
...
def export(n,build,iteration):
 ...
 if n==2:
  ...
  for name in ('price_to_sum_1','gae_overpass_1','observed_feedback_2'):
 ...
 if n==2:freeform_loop(sl,[(391,207),(394.5,207),(394.5,235),(1.3,235),(1.3,31),(23,31)])
 ...
 stem=f'Figure{n}_Identity_R1';ppt=out/(stem+'.pptx');prs.save(ppt)

...
RENDER=ROOT/'render_r3'
# Anonymous metadata on the editable sources.
for ppt in [ROOT/'Figure1_NIPSStyle_R2.pptx', ROOT/'Figure2_NIPSStyle_R2.pptx']:
...
for n in [1,2]:
    ...
    records.append({'figure':n,'pptx':f'Figure{n}_NIPSStyle_R2.pptx','pdf':outpdf.name,'svg':(ROOT/(stem+'.svg')).name,'png':outpng.name,'size_pt':[float(page.rect.width),float(page.rect.height)],'sha256':hashlib.sha256(outpdf.read_bytes()).hexdigest()})

# Font scan of editable text runs. Outlined legacy glyph groups remain part of the faithful NIPS baseline.
font={}
for n in [1,2]:
 ppt=ROOT/f'Figure{n}_NIPSStyle_R2.pptx'; prs=Presentation(str(ppt)); vals=[]; authored=[]
 def rec(sh):
...
with zipfile.ZipFile(ROOT/'NIPSStyle_R2_sources.zip','w',zipfile.ZIP_DEFLATED) as z:
    for p in [ROOT/'Figure1_NIPSStyle_R2.pptx',ROOT/'Figure2_NIPSStyle_R2.pptx',ROOT/'build.py',ROOT/'package.py',ROOT/'font_report.json',ROOT/'manifest.json',ROOT/'HANDOFF.md',ROOT/'PROCESS_NOTES.md']+list(ROOT.glob('icon_*.png')):
        z.write(p,p.name)

...
def build_fig1(src, dst):
    ...
    add_text(slide, 2.20, 0.17, 1.35, 0.10, 'one flow · all budgets', size=7.0, color=GREEN, bold=True, name='safeot_center_subtitle')
    ...
    for nm, fn in zip(['costicon0','costicon1','costicon2','costicon3'], ['icon_graph_r2.png','icon_flow_r2.png','icon_price_r2.png','icon_shield_r2.png']):
...
def build_fig2(src, dst):
    ...
    set_text(prs, 'title3', 'Soft / hard', size=8.8, bold=True, color=GREEN)
    ...
    set_text(prs, 'iterations', 'reroute F̂ → F★  |  λ★', size=7.0, bold=True, color=GREEN)
    ...
    set_text(prs, 'ppo_label', 'priced advantage', size=7.0, bold=True, color=ORANGE)
    ...
    add_text(slide, 1.38, 1.93, 0.50, 0.12, 'flow  F̂', size=7.0, color=BLUE, bold=True, name='current_flow_key')
    add_text(slide, 1.38, 2.25, 0.50, 0.12, 'adv.  A', size=7.0, color=RGBColor(80,40,130), bold=True, name='current_adv_key')
    add_text(slide, 1.38, 2.57, 0.50, 0.12, 'cost  Cₖ', size=7.0, color=GREEN, bold=True, name='current_cost_key')
    # Keep the NIPS-native rollout and actor pictograms in Figure 2 for the stronger visual anchors.
    prs.save(str(dst))


if __name__ == '__main__':
    build_fig1(ROOT / 'Figure1_NIPSStyle_R2.pptx', ROOT / 'Figure1_NIPSStyle_R2.pptx')
    build_fig2(ROOT / 'Figure2_NIPSStyle_R2.pptx', ROOT / 'Figure2_NIPSStyle_R2.pptx')
    print('built', ROOT)

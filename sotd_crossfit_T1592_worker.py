...
def update(data,*,total_step=0):
 ...
 cycle=len(events);oldactor=copy.deepcopy(agent.actor.state_dict());oldopt=copy.deepcopy(agent.actor_optimizer.state_dict());save(agent,out/f'cycle_{cycle:03d}_old.pt');torch.save(data,out/f'cycle_{cycle:03d}_batch.pt');shadow=copy.deepcopy(agent);grads=[]
 ...
 for met in ['KL','W2']:
  ...
  ji=joint_update(joint,data,B,metric=met,scale=j['scale'],c_w=c_w,estimator=j['estimator']);torch.save(joint.audit_reward_baseline,out/f'cycle_{cycle:03d}_{met}_baseline.pt');joint_shadows[met]=joint;joint_infos[met]=ji
 ...
 chosen,selection_recovery,selection_risks=choose(sample_sets,B,mode,eligible);scores=[float(x[0]) for x in selection]
 va=[];vo=[]
 ...
 accepted,recovery,oldrisk,newrisk=accept(vo,va,B,mode);va=np.mean(va,0);vo=np.mean(vo,0)

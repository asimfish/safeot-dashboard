...
## FR3 camera-calibration ablation — 2026-09-15 05:24 CST

FR3 minus_K seed 10000 completed and passed all closed-recording checks. It is a valid failure solely because final bottle COM horizontal distance to target was 0.04349269341021848 m, beyond the unchanged 0.04 m criterion by 3.4927 mm. The bottle was lifted 6.1577 cm, released, upright and stable; no geometry abort or provider error occurred. Astra submitted completion on request 17. Four first-input images and full videos are inline, with a separate original terminal frame and evaluator-only geometric diagram explaining why the visible placement misses the criterion. The diagram/target truth were not model inputs. No scoring threshold was changed.

This episode used 106904 input / 26274 output tokens and $2.38274 equivalent cost, zero unknown use. With no success in this group, cost per success is undefined. Raw collection was 1513.533 s, first-observation-to-terminal 1174.303 s, pre-observation 339.229 s; median inter-output interval 67.501 s over sixteen intervals. Videos retain 688 frames at 15 fps, 640×480; web total 2.384 MB. Its actual initial physics exactly matches the previously verified minus_S seed10000 reset. It provides S and J, removes K, and retains other common protocol inputs. One trajectory supports completion of coarse manipulation without K under this controller and other inputs; it does not prove that missing K caused the placement error.

Verified progress at this snapshot is FR3 4/140 (one success) and PsiBot 1/140 (one success): 5/280 complete, 275 pending. FR3 has entered minus_F. PsiBot has executed eleven consecutive model decisions since its independent-tunnel recovery and remains in minus_S. Neither collection policy nor control/physics was changed in this update.

30109 was rechecked through the pre-existing pinned direct-endpoint key. The named-host alias has a stale key; its check was not disabled or deleted. The direct alias's /dev/null known-host setting was overridden with the existing real known_hosts file and strict verification. Eight GPUs have foreign SafeTransport/VLLM workloads. System NVIDIA Vulkan ICD and the matched Isaac environment remain absent. Temporary process-local ICD tests of the existing 580.65.06 graphics bundle fail to obtain vkCreateInstance, both with and without glibc compatibility preload. Early memfd-ICD probes were invalid because the loader skipped the path; the corrected .json-file probe establishes the reported failure. No shared driver/environment changes, foreign process stops, model requests or formal collection were made there.

The project experiment-monitor checkpoint remains partial because its broad legacy scan reaches a file limit and finds old ERROR markers. The exact new episode marker and closed artifacts were checked separately; those global marker counts are not used as this campaign's results.

Validation: all 20 campaign clips and two historical clips play and release previous resources; desktop/mobile no page overflow and no startup MP4 request. The new terminal image and metric diagram load; desktop screenshot reviewed. Four new poster hashes, 688-frame metadata and MP4 faststart checks pass; exact metric distance recomputation matches evaluation. git diff --check and agentctl manual check pass.

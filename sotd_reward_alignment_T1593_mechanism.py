...
raw_train,pen_train=recorded_raw(src,cy);jd=dict(data,raw_rewards=raw_train);trainvalues=torch.cat([data['rewards'][:,None],raw_train[:,None],pen_train[:,None],data['costs']],1).reshape(20,12,-1);traintarget=targets(trainvalues)
for est in ['shaped','raw']:
 for metric in ['KL','W2']:
  label=est+metric;x=copy.deepcopy(agent);x.actor_optimizer=torch.optim.SGD(x.actor.parameters(),lr=1);ii=update(x,jd,B,metric=metric,c_w=cw,actor_reward_target=est);info[label]=ii;actors[label]=copy.deepcopy(x.actor)
  if est=='shaped':assert same(x.actor.state_dict(),load(src/f'cycle_{cy:03d}_{metric}.pt')['actor'])
  torch.save(dict(actor=x.actor.state_dict(),D=x.audit_directions,baseline=x.audit_reward_baseline,solver=ii),out/(label+'.pt'));ratio=lr(x.actor,agent.actor,data['states'],data['actions']).reshape(20,12);train[label]=contribution(ratio,traintarget).mean(0)
assert torch.equal(load(out/'shapedKL.pt')['D'][:,1:],load(out/'rawKL.pt')['D'][:,1:]);assert torch.equal(load(out/'rawKL.pt')['D'],load(out/'rawW2.pt')['D']);assert torch.equal(load(out/'shapedKL.pt')['D'],load(out/'shapedW2.pt')['D'])
initial={k:copy.deepcopy(a.state_dict()) for k,a in actors.items()};arrays={};stats={};base=3830000000+b['anchor']*10000;seeds=list(range(base,base+64))
...
old=arrays['sample_old'];target=targets(old['values']);draws=np.random.default_rng(159300+b['anchor']).integers(0,N,(512,N));results={}
for label in list(actors)[1:]:
 ...
 for idx in draws:
  h=contribution(ratio[idx],target[idx]).mean(0);s=sc[idx].mean(0);d=dc[idx].mean(0);boots.append(np.stack([train[label]-s,train[label]-d,s,d,h]))
 boots=np.array(boots);np.savez_compressed(out/(label+'_contributions.npz'),hold_timeLOO=hc,sample=sc,det=dc,ratio=ratio,bootstrap=boots,bootstrap_indices=draws);results[label]=dict(Gtrain=train[label].tolist(),Gsample=sc.mean(0).tolist(),Gdet=dc.mean(0).tolist(),Ghold_timeLOO=hc.mean(0).tolist(),optimism_sample=(train[label]-sc.mean(0)).tolist(),optimism_det=(train[label]-dc.mean(0)).tolist(),hold_contribution_std=hc.std(0,ddof=1).tolist(),sample_contribution_std=sc.std(0,ddof=1).tolist(),bootstrap_ci95=np.quantile(boots,[.025,.975],axis=0).tolist(),bootstrap_groups=['optimism_sample','optimism_det','Gsample','Gdet','Ghold'],channels=['shaped','raw','penalty']+['cost_'+str(i) for i in range(len(B))])
assert steps==7680 and all(same(initial[k],a.state_dict()) for k,a in actors.items());assert sourcehash=={p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in sourcehash};r=dict(status='verified',anchor=b,source_hashes=sourcehash,c_W=cw,solver=info,stats=stats,gains=results,steps=steps,episodes=640,training_steps=0,common_hold_estimator='timeLOO separately raw/shaped/penalty/cost, identical for all candidates',CRN=True,source_unchanged=True,seconds=time.time()-start);(out/'result.json').write_text(json.dumps(r,indent=2));print(json.dumps(dict(status='verified',seconds=time.time()-start)),flush=True)
